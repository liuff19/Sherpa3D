...
class _MLP(torch.nn.Module):
    def __init__(self, cfg, loss_scale=1.0):
        ...
        for i in range(cfg['n_hidden_layers']-1):
            net = net + \
                (torch.nn.Linear(
                    cfg['n_neurons'], cfg['n_neurons'], bias=False), torch.nn.ReLU())
        net = net + \
...
class Decoder(torch.nn.Module):
    ...
    def forward(self, p):
        _texc = (p.view(-1, 3) - self.AABB[0][None, ...]) / \
    ...
    def pre_train_ellipsoid(self, it, scene_and_vertices):
        ...
        points_surface_disturbed = points_surface + \
            np.random.normal(loc=0.0, scale=0.05,
                             size=points_surface.shape).astype(np.float32)
        point_rand = (np.random.rand(3000, 3).astype(
            np.float32)-0.5) * self.mesh_scale
        query_point = np.concatenate(
            (points_surface, points_surface_disturbed, point_rand))
